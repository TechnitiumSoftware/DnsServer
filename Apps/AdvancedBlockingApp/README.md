...
A DNS App (plugin) for [Technitium DNS Server](https://technitium.com/dns/) that provides advanced domain blocking capabilities with support for client-based group policies, multiple block list formats, and fine-grained control over blocking behavior.
...
- **Parental Controls**: Create a "kids" group with stricter blocking for children's devices
- **Guest Network**: Apply different policies to guest WiFi subnet
- **IoT Isolation**: Block telemetry for IoT devices on a specific VLAN
- **Multi-tenant DNS**: Different blocking policies for different clients sharing the same DNS server
- **DoH/DoT Differentiation**: Apply different policies based on DNS transport protocol
...
This app is part of the Technitium DNS Server project and is licensed under the GNU General Public License v3.0. 