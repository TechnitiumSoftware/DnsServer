...
describe('api/settings', () => {
  it('settings/get asks with an empty node and returns the content of `response`', async () => {
    ...
    const call = spy.mock.calls.find((c) => c[0] === 'settings/get')!
  ...
  it('settings/set goes by POST with the body as it is', async () => {
    ...
    const call = spy.mock.calls.find((c) => c[0] === 'settings/set')!
  ...
  it('settings/forceUpdateBlockLists is called with no parameters', async () => {
    ...
    const call = spy.mock.calls.find((c) => c[0] === 'settings/forceUpdateBlockLists')!
    expect(call[1]).toEqual({ token: 'tok' })
  })
  ...
  it('settings/temporaryDisableBlocking sends the minutes and returns until when', async () => {
    ...
    const call = spy.mock.calls.find((c) => c[0] === 'settings/temporaryDisableBlocking')!
    expect(call[1]).toMatchObject({ body: { minutes: '15' } })
  ...
  it('cache/flush sends an empty node', async () => {
    ...
    const call = spy.mock.calls.find((c) => c[0] === 'cache/flush')!
    expect(call[1]).toMatchObject({ body: { node: '' } })
  })
  ...
  it('settings/restore sends the file by multipart and the options in the query', async () => {
    ...
    expect(call[0]).toContain('logs=false')
    expect(call[0]).toContain('authConfig=true')
    expect(call[1]).toMatchObject({
      method: 'POST',
      token: 'tok',
